...
type User struct {
	...
	Role      string    `gorm:"not null" json:"role"` // role == 1 --> admin
	Job       string    `gorm:"not null"  json:"job"` // job = 1 : giáo viên
	Avartar   string    `gorm:"size:100"   json:"avartar"`
...
func (u *User) Prepare() {
	u.ID = 0
	u.Role = "0" // user : not admin
	u.Job = "0"  // user not teacher
